...
def permutation_test_between_clfs(y_true, pred_probas_1, pred_probas_2, scoring="roc_auc", n_repeats=1000):
    ...
    pred_probas_1 = np.asarray(pred_probas_1).ravel()
    pred_probas_2 = np.asarray(pred_probas_2).ravel()

    if scoring == "roc_auc":
        score1 = roc_auc_score(y_true, pred_probas_1)
        score2 = roc_auc_score(y_true, pred_probas_2)

    elif scoring == "average_precision":
        score1 = average_precision_score(y_true, pred_probas_1)
        score2 = average_precision_score(y_true, pred_probas_2)
    ...
    for _ in range(n_repeats):
        mask = np.random.randint(2, size=len(pred_probas_1))
        p1 = np.where(mask, pred_probas_1, pred_probas_2)
        p2 = np.where(mask, pred_probas_2, pred_probas_1)

        if scoring == "roc_auc":
            score1 = roc_auc_score(y_true, p1)
            score2 = roc_auc_score(y_true, p2)
